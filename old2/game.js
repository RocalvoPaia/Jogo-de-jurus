  ...
  const html =
    `<div class="modal-grid cols-2">
      <div class="term-card">
        ...
        <div class="log" style="max-height:200px">${history || '<div class="log-item info">Sem histórico disponível.</div>'}</div>
      </div>
      <div class="term-card">
...
function unlockTerms(keys, sourceTitle) {
  ...
  // Delay term popup so event renders first and isn't immediately overwritten
  if (newTerms.length) {
...
function monthlyTick() {
  const monthRevenue = state.revenue * (0.85 + Math.random() * 0.3);
  const extraExpense = Math.random() < 0.18 ? Math.round(5000 + Math.random() * 10000) : 0;
  const profit = monthRevenue - state.expenses - extraExpense;

  state.cash += profit;
  addLog(`Mês ${state.month}: faturamento ${fmt(monthRevenue)} e resultado operacional ${fmt(profit)}.`, extraExpense ? "bad" : "info");
  if (extraExpense) {
    addLog(`Custos inesperados de R$ ${extraExpense.toLocaleString("pt-BR")} pressionaram o caixa.`, "bad");
  }
  ...
  if (state.debt > 0 && state.debtTerms > 0) {
    ...
    if (payment < due) {
      ...
      const penalty = Math.round(shortfall * 0.15);
      state.debt += penalty;
      addLog(`Caixa insuficiente para pagar a parcela completa. Juros e multa adicionam ${fmt(penalty)} à dívida.`, "bad");
      state.creditScore = Math.max(0, state.creditScore - 35);
    }
  }
  ...
  if (state.cash < 0) {
    state.cash = 0;
    state.gameOver = true;
    addLog(`Mês ${state.month}: caixa zerado — empresa faliu.`, "bad");
  }
...
function startGame() {
  ...
  state.debtRate = 0.04;
  state.debtTerms = 12;
  state.creditScore = 550;
  state.revenue = 72000;
  state.expenses = 76000;
  state.supplierRep = 60;
  state.clientTrust = 65;
  state.invested = 0;
  state.investRate = 0;
